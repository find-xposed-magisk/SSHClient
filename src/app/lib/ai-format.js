...
export const FORMAT_OPENAI_CHAT = 'openai-chat'
export const FORMAT_OPENAI_RESPONSES = 'openai-responses'
...
//   /messages         -> anthropic
export function detectFormat (path, format) {
...
export function headersForFormat (format) {
...
export function toOpenAIToolCall ({ id, name, arguments: args }) {
  return {
    id: id || '',
    type: 'function',
    function: {
...
export function buildRequest (format, { model, messages, tools, stream, maxTokens } = {}) {
...
export function createStreamParser (format) {
  const state = {
    content: '',
    completed: false,
    error: null
...
export function streamResultToMessage (parser) {
  const message = {
    role: 'assistant',
